...
import xyz.block.goosemobile.features.agent.ToolHandler.callTool
import xyz.block.goosemobile.features.agent.ToolHandler.getSerializableToolDefinitions
...
class Agent : Service() {
    ...
    suspend fun processCommand(
        ...
        try {
            ...
            val availableIntents = IntentScanner.getAvailableIntents(
                context,
                GooseMobileAccessibilityService.getInstance()
            )
    ...
    private fun executeTools(
        ...
        val results = toolCalls.mapIndexed { index, toolCall ->
            ...
            val startTime = System.currentTimeMillis()
            val result = callTool(toolCall, context, GooseMobileAccessibilityService.getInstance())
